...
module.exports.fragments_MIDDLEWARE = function(fragments_hinoki, fragments_applicationLifetime, fragments_RequestLifetime, fragments_MiddlewareLifetime) {
  return function(middlewareFactory) {
    return function(req, res, next, params) {
      ...
      return fragments_hinoki.get([new fragments_MiddlewareLifetime(next, params), req.hinokiRequestLifetime, fragments_applicationLifetime], fragments_hinoki.getAndCacheNamesToInject(middlewareFactory)).spread(middlewareFactory)["catch"](function(err) {
        console.log('ERROR');
        console.log(err);
